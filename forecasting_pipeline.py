...
def extrapolation_warning(regime,want,have):
    print("WARNING: if extrapolation is permitted during interpolation, it will be conducted for {:15s} (want {:9.4}, have{:9.4})".format(regime,want,have))
...
# Fourier space
def kpar(nu_ctr=600*u.MHz,chan_width=CHORD_channel_width_MHz,N_chan=300,H0=H0): # not pure cosmo. relies on LoS details of survey
...
class beam_effects(object):
    def __init__(self,
                 ...
                 # CONVENIENCE
                 heavy_beam_recalc:bool=True,         # save time by using pre-saved beams?
                 ):   
        ...
        _, \
        self.Nchan, self.synth_comoving_extent, _, \
        _, \
        _, _, \
        _, _ = discretize_LoS(self.nu_lo, self.nu_hi, delta_nu) # last one was P S F_flat_sky_comoving_centred
        ...
        self.synth_Nz=self.Nchan
        print("beam_effects.__init__: self.synth_Nz=",self.synth_Nz)
        ...
        CSTsynth_xy_ext=2*transverse_half_angle*self.comoving_mid # stricter than the horizon limit. 2x is to account for +/- approved-small-angle from boresight
        CSTsynth_xy_vec= CSTsynth_xy_ext*fftshift(fftfreq(Npix))
        self.Npix=Npix
        kperpmin_synthCST=twopi/CSTsynth_xy_ext
        kperpmax_synthCST=Npix/CSTsynth_xy_ext*pi
        kparmin_synth=twopi/self.synth_comoving_extent
        kparmax_synth=self.Nchan/self.synth_comoving_extent*pi
        print("synth k-perp extent:",kperpmin_synthCST,kperpmax_synthCST)
        print("synth k-par  extent:",kparmin_synth,kparmax_synth)
        ...
        if beam_sim_directory!="":
            ...
            if CST_f_head_syst=="": # prevent redundant CST translation
                already_imported_syst_CST=True
            middle_name=str(CST_lo.value)+"_"+str(CST_hi.value)+"_"+str(CST_deltanu.value)
            ...
            if heavy_beam_recalc and not already_imported_syst_CST: # only import the fiducial beam once
                for i,CST_f_head_syst_i in enumerate(CST_f_head_syst):
                    print("reimporting syst ensemble")
            ...
            CST_domain=(CSTsynth_xy_vec,CSTsynth_xy_vec,CST_z_vec)
            self.CST_z_vec=CST_z_vec
            self.CSTsynth_xy_ext=CSTsynth_xy_ext
            ...
            if heavy_beam_recalc: # redo the beam synthesis
                fidu_synthesis=simulate_array(array_version=array_version,N_timesteps=self.N_timesteps,N_hrs=N_hrs,
                                                    ...
                                                    CSTsynth_xy=CSTsynth_xy_vec,CST_freqs=CST_freqs,
                                                    supplementary_name=ioname)
                fidu_synthesis.stack_to_box()
                print("finished synthesizing fiducial CST synth beam")
                fidu_box_MM=fidu_synthesis.MM
                if N_CST_types>1 or N_pointing_errors_max>0:
                    ...
                    syst_box_MM=syst_synthesis.MM
                    weights_synth=syst_synthesis.weights
                    Ntypes=syst_synthesis.N_total_beam_types
                else:
                    syst_box_MM=np.copy(fidu_box_MM)
                    weights_synth=fidu_synthesis.weights
                    Ntypes=1
                ...
                fidu_box_MM=np.load("fidu_box_MM_"+ioname+".npy")
                syst_box_MM=np.load("syst_box_MM_"+ioname+".npy")
                weights_synth=np.load("weights_synth_"+ioname+".npy")
                print("loaded synthesized beam")
            print("finished importing/constructing synthesized CST beam")
            print("fidu_box_MM.shape=",fidu_box_MM.shape)
            
            weighted_sum_syst_primary=np.zeros_like(fidu_box)
            Ntypes=len(weights_synth) # this is super hacky and I need to streamline it
            if Ntypes>1:
                q=0
    ...
    def get_pwr_law_FG_ingredient(self,              # synchrotron and free-free FG emission are both well described by power laws. cf. Liu & Tegmark 2011 (https://arxiv.org/abs/1106.0007)
        ...
        fg=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_Delta_z,
    ...
    def calc_power_contamination(self, isolated:bool=False): # Monte Carlo numerical windowing of beam-aware brightness temp boxes to yield several cylindrically power spectra of interest for forecasting and diagnostics. various states of beam knowledge and fiducial spectrum as appropriate (see Memos I-II)
        ...
        if self.layer_foregrounds:
            ...
            for fg_info in tqdm(fg_info_cases):
        ...
        co_fi_sy_fg=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_comoving_extent,
                                P_fid=P_cosmo,k_fid=self.ksph,
                                Nxy=self.Npix,Nz=self.synth_Nz,
                                MM=self.systMM,# MM2=self.fiduMM,
                                frac_tol=self.frac_tol_conv,seed=self.seed,
                                LoS_apo=self.LoS_apo,transverse_apo=self.transverse_apo,
                                wedge_cut=self.wedge_cut,nu_ctr=self.nu_ctr,fg_box=fg_box)
        xx_fi_sy_fg=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_comoving_extent,
                                Nxy=self.Npix,Nz=self.synth_Nz,
                                T_pristine=fg_box,
                                MM=self.systMM,# MM2=self.fiduMM,
                                frac_tol=self.frac_tol_conv,seed=self.seed,
        ...
        xx_fi_xx_fg=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_comoving_extent,
                                Nxy=self.Npix,Nz=self.synth_Nz,
                                T_pristine=fg_box,
                                MM=self.fiduMM,
                                frac_tol=self.frac_tol_conv,seed=self.seed,
                                LoS_apo=self.LoS_apo,transverse_apo=self.transverse_apo,
                                wedge_cut=self.wedge_cut,nu_ctr=self.nu_ctr)
        co_fi_xx_xx=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_comoving_extent,
                                P_fid=P_cosmo,k_fid=self.ksph, 
                                Nxy=self.Npix,Nz=self.synth_Nz,
                                MM=self.fiduMM,
                                frac_tol=self.frac_tol_conv,seed=self.seed,    
                                LoS_apo=self.LoS_apo,transverse_apo=self.transverse_apo,
                                wedge_cut=self.wedge_cut,nu_ctr=self.nu_ctr)
        co_fi_sy_xx=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_comoving_extent,
                                P_fid=P_cosmo,k_fid=self.ksph, 
                                Nxy=self.Npix,Nz=self.synth_Nz,
                                MM=self.systMM, #MM2=self.fiduMM,
                                frac_tol=self.frac_tol_conv,seed=self.seed,    
                                LoS_apo=self.LoS_apo,transverse_apo=self.transverse_apo,
                                wedge_cut=self.wedge_cut,nu_ctr=self.nu_ctr)
        co_xx_xx_fg=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_comoving_extent,
                                P_fid=P_cosmo,k_fid=self.ksph, 
                                Nxy=self.Npix,Nz=self.synth_Nz,
                                frac_tol=self.frac_tol_conv,seed=self.seed,    
                                LoS_apo=self.LoS_apo,transverse_apo=self.transverse_apo,
        ...
        COSMOTEST=cosmo_stats(self.CSTsynth_xy_ext,Lz=self.synth_comoving_extent,
                              P_fid=P_cosmo,k_fid=self.ksph, 
                              Nxy=self.Npix,Nz=self.synth_Nz,
                              LoS_apo=self.LoS_apo,transverse_apo=self.transverse_apo,
...
class cosmo_stats(object):
    def __init__(self,
                 ...
                 MM:np.ndarray=None, MM2=None,                             # synth beam (box of values evaluated in config space); and white noise map for normalization
        ...
        if ((T_with_beam is None) and (T_pristine is None) and (P_fid is None) and (MM is None)): # require either a box or a fiducial power spec (il faut some way of determining #voxels/side; passing just Nxy is not good enough)
            ...
            if ((T_pristine is not None) and (T_with_beam is not None)):
                print("WARNING: T_pristine and T_with_beam both passed; T_with_beam will be temporarily ignored and then internally overwritten to ensure consistency with beam")
                if (T_pristine.shape!=T_with_beam.shape):
                    raise ValueError("conflicting info")
                else:                                                          # use box shape to set cubic/ rectangular prism box attributes
                    Nxy, _, Nz=T_with_beam.shape
            if ((Nxy is not None) and (T_pristine is not None)):              # possible conflict: if both Nxy and a box are passed, 
                T_pristine_shape0,_,T_pristine_shape2=T_pristine.shape
                if (Nxy!=T_pristine.shape[0]):                                # but Nxy and the box shape disagree,
                    raise ValueError("conflicting info")                       # estamos en problemas
                else:
                    Nxy= T_pristine_shape0                               # otherwise, initialize the Nxy attributes
                    Nz=T_pristine_shape2
            elif (Nxy is not None and Nz is None):                                           # if Nxy was passed but T was not, use Nxy to initialize the Nxy attributes
                Nz=np.copy(Nxy)
            elif T_pristine is not None:                                                              # remaining case: T was passed but Nxy was not, so use the shape of T to initialize the Nxy attributes
                Nxy, _, Nz = T_pristine.shape
            self.Nxy=Nxy
            self.Nz=Nz
            ...
            if (P_fid is not None): # no hi fa res si the fiducial power spectrum has a different dimensionality or bin width than the realizations you plan to generate (boxes will be generated from a grid-interpolated P_fid, anyway)
                ...
                if (Pfiddims==2):
                    if MM is None: # trying to do a minimalistic instantiation where I merely provide a fiducial power spectrum and interpolate it
        ...
        # beams
        MMuse=np.ones((self.Nxy,self.Nxy,self.Nz))
        self.MM=None
        if MM is not None:
            self.MM=MM
            MMuse=MM
    ...
    def resample_P_fid_on_grid(self): # resample a 1D power spec on a 3D grid. to break these symmetries, you can do a bit of reverse-engineering: do what you want to the box -> update the T_pristine attribute -> form a power spec using the same cosmo_stats object -> save that unbinned power spec as P_fid_grid -> continue with your Monte Carlo or whatever
        ...
        P_fid_flattened_box=np.zeros(self.Nxy**2*self.Nz)
    ...
    def generate_P(self,T_use=None): # from a box of temperature field values
        if T_use is None:            # establish common string flags
            if self.MM is None:
                ...
                T_use="beam"
        if (T_use.lower()=="beam"):  # do the correct thing based on the updated string flag
            T_use=None
            if self.T_with_beam is None:
    ...
    def bin_power(self,power_to_bin=None):
        ...
        power_to_bin_flat=np.reshape(power_to_bin,(self.Nxy**2*self.Nz,))
    ...
    def power_Monte_Carlo(self,interfix:str=""): # since box generation is not deterministic
        ...
        t0=time.time()
        for i in tqdm(range(self.N_realizations)):
            self.generate_GRF()
...
def beam_type_distribution(N_NS,N_EW,N_types,distribution="random",frame_width=2):
    N_ant=N_NS*N_EW
    print("beam_type_distribution: N_types=",N_types)
    if N_types>0:
        rng=np.random.default_rng()
...
class simulate_array(beam_effects): # developed with rectangular arrays in mind
    def __init__(self,
        ...
        surv_channels_Hz=surv_channels_MHz.to(u.Hz) # still decreasing
        surv_wavelengths=c/surv_channels_Hz # increasing
        self.surv_wavelengths=surv_wavelengths.decompose()
        self.comoving_ctr=comoving_ctr
        self.lambda_obs=surv_wavelengths[0]

        # helper args
        self.CSTsynth_xy=CSTsynth_xy
        self.Deltaxy=CSTsynth_xy[1]-CSTsynth_xy[0]
        N_CSTsynth_xy=len(CSTsynth_xy)
        self.CST_freqs=CST_freqs
        self.CST_deltanu=CST_freqs[1]-CST_freqs[0]
        self.N_CSTsynth_xy=N_CSTsynth_xy
    ...
    def calc_uv_slice(self):
        ...
        sampling_function,_,_=np.histogram2d(np.reshape(self.uv_synth[:,0,:],N_tot,order="C"),
                                              np.reshape(self.uv_synth[:,1,:],N_tot,order="C"),
                                              bins=self.uvbins_use)
        adjust_weights=np.ones((self.Npix,self.Npix))
        adjust_weights[sampling_function!=0]=sampling_function[sampling_function!=0]
        k=0
        for i in range(self.N_total_beam_types):
            ...
            for j in range(i+1):
                ...
                sampling_ij,_,_=np.histogram2d(reshaped_u,reshaped_v,bins=self.uvbins_use) # naïve gridding kernel
                comb=np.nonzero(sampling_ij) # uv grid points where there are baseline(s)
                if self.weighting=="custom":
                    sampling_ij[comb]=1/sampling_ij[comb]
...
class reconfigure_CST_beam(object):
    ...
    def construct_CST_box(self):
        ...
        for i in tqdm(range (self.Nfreqs)):
            if self.multi_CST:
...
class CHORD_sense(object): # modified from a notebook helpfully shared by Debanjan Sarkar in April 2025
    def __init__(
        ...
        channel_width=CHORD_channel_width_MHz,
        ...
        self.n_days = n_days
        n_channels = bandwidth.value/channel_width
...
def power_comparison_plots(redo_window_calc:bool=False, redo_box_calc:bool=False,
              array_version:str="pathfinder", nu_ctr:float=800,
    ...
    for i,complexity_type in enumerate(complexity_cases):
        ...
        if Npoint_i==0 and NCST_i==1:
            CST_f_head_syst_i="" # literally just use fiducial for both num and denom everywhere
        observation=beam_effects(# SCIENCE
                                    # the observation
                                    bminCHORD,bmaxCHORD,                                                       
